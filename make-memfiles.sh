...
riscv32-unknown-elf-objcopy -O binary --wildcard -j .rodata -j .rodata.* -j .data -j .data.* -j .bss -j .bss.* -j .sdata -j .sbss --set-section-flags=.bss=alloc,load,contents $SRC.elf $SRC.data.bin
